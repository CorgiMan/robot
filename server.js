...
function handleMessage(data) {
    ...
    if (data.type === 'kinematic-inversion') {
        ...
        // todo(wieger): need better collision prevention (could simulate the robot movement)
        if (Math.max(Math.abs(gripperX), Math.abs(gripperZ)) < 400) {
            // todo(wieger): notify user
            console.log('kinematic inversion failed, gripper is too close to the lift');
            console.log('gripperX', gripperX, 'gripperZ', gripperZ);
            return;
        }
